...
import { User as UserEntity } from "@/lib/kernel/models/main/User";
...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    register({ id: Math.random().toString(36).slice(-5), email: email, password: password } as UserEntity);
  };
  ...
  const handleGoogle = useGoogleLogin({
    onSuccess: (codeResponse) => {
      axios.get(`https://www.googleapis.com/oauth2/v1/userinfo?access_token=${codeResponse.access_token}`, {
        ...
        setLoading(true);
        login({ id: res.data.id, login: res.data.email, method: AUTH_TYPE.GOOGLE } as UserEntity);
  ...
  useEffect(() => {
    ...
    if (user?.ready) {
      ...
      if (user.method == AUTH_TYPE.GOOGLE) {
        login({ id: user.googleId, login: user.login, method: AUTH_TYPE.GOOGLE } as UserEntity);
      }
    }
  }, [user]);

  const inputClass =
    "w-full h-11 rounded-xl border border-border bg-background pl-11 pr-4 text-sm text-foreground placeholder:text-text-hint focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30 transition-colors";
  const inputWithToggle =
    "w-full h-11 rounded-xl border border-border bg-background pl-11 pr-11 text-sm text-foreground placeholder:text-text-hint focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30 transition-colors";
