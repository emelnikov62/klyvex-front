...
import {CreditCard, FolderOpen, HelpCircle, LogIn, LogOut, Menu, Monitor, Settings, Sparkles, User, X} from "lucide-react";
                ...
                <span className="text-sm font-bold text-foreground">
                  KLYVEX<span className="text-primary">.AI</span>
                                ...
                                <button onClick={() => setBurgerOpen(false)}
                                        className="h-8 w-8 flex items-center justify-center rounded-lg text-muted-foreground active:bg-surface-hover">
                                    <X className="h-4 w-4"/>
                                </button>
                            </div>
                            ...
                            <div className="px-4 py-4 border-b border-border">
                                {isAuthenticated && user ? (
                                    <div className="flex items-center gap-3">
                                        <div
                                            className="h-10 w-10 rounded-xl bg-primary/20 border border-primary/30 flex items-center justify-center text-primary font-bold">
                            ...
                            <nav className="flex-1 overflow-y-auto py-2">
                                {navItems.map((item) => {
                                    const isActive = locationVar.pathname === item.path;
                                    return (
                                        <button
                                            key={item.path}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        {isAuthenticated ? (
                            <div className="relative" ref={balanceRef}>
                                ...
                                >
                                    <CloverIcon
                                        className="h-3.5 w-3.5 text-primary group-hover:drop-shadow-[0_0_8px_rgba(200,255,0,0.4)] transition-all"
                                        fill="currentColor"/>
                                    <span className="text-xs font-semibold text-foreground">1 500</span>
                                </button>
                                {balanceOpen && (
                                    <BalancePopup onClose={() => setBalanceOpen(false)} onTopUp={() => {
                                        setBalanceOpen(false);
                                        setTopUpOpen(true);
                                    }}/>
                                )}
                        ...
                        {isAuthenticated && user ? (
                            <div className="relative" ref={dropdownRef}>
                                <button
                                    onClick={() => {
                                        if (dropdownOpen) setDropdownOpen(false); else {
                                            setBalanceOpen(false);
                                ...
                                {dropdownOpen && (
                                    <div
                                        className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-border bg-card shadow-xl shadow-background/50 overflow-hidden popup-enter">
                                        ...
                                        <div className="py-1">
                                            <button onClick={() => {
                                                ...
                                                navigate("/settings");
                                                    ...
                                                    className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-muted-foreground hover:text-foreground hover:bg-surface-hover transition-colors">
                            ...
                            <button onClick={() => navigate("/login")}
                                    className="flex items-center gap-2 rounded-lg border border-primary/40 px-4 py-1.5 text-sm font-medium text-foreground hover:border-primary hover:bg-primary/10 transition-all">
                                <LogIn className="h-4 w-4"/>
